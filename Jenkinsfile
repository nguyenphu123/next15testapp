    ...
    stages {
        ...
        stage('Deploying container to Kubernetes') {
            steps {
                ...
                sh "sudo microk8s kubectl expose deployment next --type=LoadBalancer --port 80 --target-port 80 --name=next-service"
                sh "sudo microk8s kubectl get all --all-namespaces"
                sh "sudo microk8s kubectl port-forward svc/next-service 8008"
                sh "sudo microk8s kubectl get all --all-namespaces"
